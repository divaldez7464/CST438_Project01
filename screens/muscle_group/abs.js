...
export default function AbsScreen({ navigation , route }) {
  ...
  const getAbsExercises = async () => {
    try {
      const exercises = {};
      for (const muscle of absMuscles) {
  ...
  return (
    <View style={styles.container}>
      <Text style={styles.title}>Abs Exercises</Text>
      {loading ? (
        <Text>Loading...</Text>
      ) : (
        <ScrollView style={styles.scrollView}>
          {absMuscles.map((muscle) => (
            <View key={muscle}>
              <TouchableOpacity 
